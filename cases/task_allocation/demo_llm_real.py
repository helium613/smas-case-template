...
python cases/task_allocation/demo_llm_real.py で(リポジトリルートから)実行する。
事前に `pip install anthropic` と、`ANTHROPIC_API_KEY`(または`ant auth login`)
による認証が必要。
...
deviation_test.run_scene に通して実行する。スタンドアロンのAPI呼び出しではなく、
①環境層・②誘因構造層(VCG)を含む実際のパイプラインにLLM実物を差し込める
ことを示す(④実行主体層のプラガブル性、CLAUDE.md 2章原則4: 型の互換性は
保証するが、振る舞いの同等性は保証しない)。
"""
...
from deviation_test import run_scene
