...
def main() -> None:
    ...
    lines.append(
        "- **発見(循環委任のキックバック、smoke_test.pyで検証)**: 循環委任(相互委任)がある場合、"
        "自分自身が受け取り側にも回ることで、委任元自身の保有額が自分自身のintended_max_budgetを"
        "超えてしまう現象を確認した——誰からも虚偽の金額を受け取っていないのに、相互委任の合成"
        "だけで自分の本来の上限を超える、ケース5には無かった新しい種類の合成リスク。"
    )
    lines.append(
