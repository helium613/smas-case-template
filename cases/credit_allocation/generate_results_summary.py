...
def main() -> None:
    ...
    lines.append(
        "- 結託耐性(#5): 本ケース固有の結託シナリオへの検証は未実施(複数エージェントが"
        "同時に共謀するケースは単独逸脱を前提とする②MDPの検証範囲外)。`pygambit`は"
        "ケース1(VCG)向けに`verification_kit/gambit_collusion.py`として実装・行使し、"
        "技術スタックに記載済みだが未使用だったギャップを解消した(D-33)。信用枠配分固有の"
        "結託シナリオ(例: 複数エージェントが交代で違反し、制裁の重複適用を回避する)への"
        "適用はまだ行っていない、引き続き別途の検証課題。"
    )
    lines.append("")
