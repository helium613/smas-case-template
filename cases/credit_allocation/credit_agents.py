...
from agents.optimization_based import ScalarBidEngine
...
class OptimizingCreditAwareAgent:
    """④実行主体層(最適化ベース、D-34の考え方をこのケースに適用、D-45)。

    ケース1のOptimizingBidderAgent(agents/optimization_based.py)は、勝てば
    「真の評価額−支払い」を得るVCGの支払い構造を前提に期待効用を最大化した。
    このケースは支払いが無く、勝てば真の評価額をそのまま得る単純な構造のため、
    期待効用は「競合の申告分布に対する勝率×真の評価額」に単純化される。

    信用枠を超える申告は探索範囲(bounds)に含めない——超過後の複数ラウンドに
    わたる制裁コストを織り込んだ多期間の動的計画(POMDP相当)は、CLAUDE.md 3章で
    「今回実装しない」と定めた範囲に踏み込むため、単一ラウンドの期待効用最大化に
    意図的に留める(D-34と同じ設計方針)。

    この制約のもとで探索が信用枠の境界(credit_limit)に収束するなら、D-37
    (敵対的LLMが発見)・D-38(CreditLimitMaximizingAgentとして再現)の
    「信用枠のすぐ下を狙う」戦略が、恣意的なルールではなく期待効用最大化の
    観点からも独立に導かれる局所最適であることを裏付けたことになる。
    """
    ...
    def _expected_utility(self, declared_value: float, competitor_bids: list[float], true_value: float) -> float:
        total = 0.0
        for competitor_bid in competitor_bids:
            declarations = [
                Declaration(agent_id=self.agent_id, declared_value=declared_value),
                Declaration(agent_id="_competitor", declared_value=competitor_bid),
            ]
    ...
    def decide(self, observation: ObservationInput) -> ActionOutput:
        ...
        if credit_limit is None or credit_limit <= 0:
            return ActionOutput(action="bid", declared_value=min(true_value, credit_limit or 0.0), reasoning=None)
        competitor_bids = [self.competitor_bid_sampler(self.rng) for _ in range(self.n_samples)]
        result = minimize_scalar(
            lambda x: -self._expected_utility(x, competitor_bids, true_value),
            bounds=(0.0, credit_limit),
            method="bounded",
        )
