...
def main() -> None:
    ...
    lines.append(
        ...
        "この「望ましくない性質」は評価観点25項目のいずれにも綺麗には対応しない、新種の"
        "性質だと確認済み(D-60)。"
    )
    lines.append("")
    lines.append("## 5大指標")
    ...
    lines.append(
        "- **対応する評価観点なし(D-60の発見)**: このケースの核心(合成による創発的リスク)は、"
        "#5結託耐性(複数主体が意図的に共謀する)とも、#12適応的逸脱への頑健性(検出ルールを"
        "学習して回避する)とも異なる——admin・ci_svc・build_svcの誰も、他者と共謀せず、"
        "検出ルールを意識してもいない。「個別に正当な意思決定の合成が生む創発的リスク」を"
        "測る評価観点は、既存の25項目に存在しない。これは25項目のカタログ自体の潜在的な"
        "空白であり、拡張候補として記録する(新規ドキュメントは作らず、ここに留める)。"
    )
    lines.append("")
